			...
			if( filePathInfo[i].folder.indexOf( "core-properties" ) > -1 
				...
				if( files ){
					for( var j=0; j<files.length; j++ ){
						...
						if( files[j].indexOf( "query.xml" ) > -1 && fs.existsSync( path.join( process.cwd(), folder.replace( "-dev", ""), "query.json" ) ) ){
							console.log( "\x1b[31m%s\x1b[0m", "[summer-mvc core]", "version control : copy file -", path.join( folder, "query.json" ) );
							common.copyFile( path.join( process.cwd(), folder.replace( "-dev", ""), "query.json"), path.join( process.cwd(), folder, "query.json") );	
						}
					}			
				}
			}
